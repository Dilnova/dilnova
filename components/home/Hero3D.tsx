...
export default function Hero3D() {
  ...
  const updateTransforms = (x: number, y: number) => {
    ...
    if (linesRef.current)
      linesRef.current.style.transform = `translate3d(${x * -15}px, ${y * -15}px, 0)`;
    if (nodesRef.current)
      nodesRef.current.style.transform = `translate3d(${x * -20}px, ${y * -20}px, 0)`;
    if (hubRef.current)
      hubRef.current.style.transform = `translate3d(${x * -35}px, ${y * -35}px, 0)`;
  };
  ...
  const nodes = [
    ...
    },
    {
      ...
      border: "border-zinc-500/30 dark:border-zinc-400/20",
      posClass: "top-[10%] right-[40%]",
    },
    {
      id: "vendor2",
      icon: Store,
  ...
  return (
    ...
    >
      ...
      <div className="max-w-7xl mx-auto w-full px-6 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center relative z-10">
        ...
        <div
          className="relative h-64 w-full md:hidden flex items-center justify-center border border-zinc-200 dark:border-zinc-800/50 rounded-2xl bg-zinc-100/50 dark:bg-zinc-900/20 overflow-hidden"
          aria-hidden="true"
        >
          {/* Static SVG Lines */}
