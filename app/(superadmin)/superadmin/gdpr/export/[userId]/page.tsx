...
export default async function GDPRDownloadPage({
  ...
  if (!file) {
    return <div className="p-8 text-center text-red-600 font-sans">Missing file parameter.</div>;
  }
  ...
  return (
    <div className="max-w-md mx-auto p-8 mt-12 bg-white dark:bg-zinc-950 rounded-xl shadow-sm border border-zinc-200 dark:border-zinc-800 text-center font-sans">
      <h1 className="text-2xl font-bold mb-4 text-zinc-900 dark:text-zinc-50">
        Secure GDPR Download
      </h1>
      <p className="text-zinc-600 dark:text-zinc-400 mb-8">
        Your export for user <strong className="font-mono text-xs break-all">{userId}</strong> is
        ready. This link expires in {Math.round(GDPR_EXPORT_TTL_SECONDS / 60)} minutes.
      </p>
