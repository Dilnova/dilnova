...
async function runBackup() {
  ...
  try {
    ...
    console.error('Please install postgresql-client (e.g. brew install postgresql or apt install postgresql-client).');
    process.exit(1);
  }
  ...
  try {
    ...
    if (bucket) {
      ...
      const endpointArg = process.env.S3_BACKUP_ENDPOINT ? `--endpoint-url ${process.env.S3_BACKUP_ENDPOINT}` : '';
      const regionArg = process.env.S3_BACKUP_REGION ? `--region ${process.env.S3_BACKUP_REGION}` : '';
