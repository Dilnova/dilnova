...
async function seed() {
  ...
  for (const val of taxValues) {
    const [tax] = await db.insert(schema.taxClasses).values(val).onConflictDoNothing().returning();
    if (tax) {
  ...
  const [vehicleTemplate] = await db
    .insert(schema.metadataTemplates)
    .values({
  ...
  const [electronicsTemplate] = await db
    .insert(schema.metadataTemplates)
    .values({
  ...
  const [livingOrganismTemplate] = await db
    .insert(schema.metadataTemplates)
    .values({
      ...
      fields: [
        ...
        { key: "breed_type", label: "Breed / Variety", type: "string", required: false },
        {
          ...
          label: "Care / Watering Instructions",
          type: "string",
          required: false,
        },
      ],
  ...
  const [cosmeticsTemplate] = await db
    .insert(schema.metadataTemplates)
    .values({
  ...
  // F. Services Template
  const [servicesTemplate] = await db
    .insert(schema.metadataTemplates)
    .values({
      ...
      fields: [
        ...
        {
          key: "location_type",
          label: "Location Type (In-store/Remote/Client)",
          type: "string",
          required: true,
        },
      ],
  ...
  const categoriesToSeed = [
    ...
    // --- ANIMALS & PLANTS ---
    {
      ...
      taxId: stdTax.id,
      children: [{ name: "Pet Supplies", slug: "animals-pet-supplies" }],
    },
    // --- FOOD & GROCERY ---
    {
      name: "Food & Grocery",
