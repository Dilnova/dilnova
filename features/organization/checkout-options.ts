...
export type { CheckoutOptionType, CheckoutOptionDefinition } from "./checkout-options.shared";
...
const fetchOrgCheckoutOptions = async (orgId: string) => {
  const fetcher = unstable_cache(
    async (id: string) => {
      ...
      if (
        meta.checkout_options &&
        typeof meta.checkout_options === "object" &&
        !Array.isArray(meta.checkout_options)
      ) {
