...
export default function ComplianceTab() {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        if (result.data.orders.length === 0 && result.data.contactSubmissions.length === 0) {
          ...
          triggerNotification(true, `Found ${result.data.orders.length} orders and ${result.data.contactSubmissions.length} contact submissions.`);
        }
      } else {
  ...
  const handleExportByUserId = () => {
    ...
    const blob = new Blob([JSON.stringify(complianceApiData, null, 2)], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
  ...
  const handleAnonymizeByUserId = async () => {
    ...
    try {
      ...
      triggerNotification(
        true,
        `API Erasure successful: ${JSON.stringify(data.deleted)}`
      );
    } catch (err: unknown) {
