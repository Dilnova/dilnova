...
export default function PricingTab({ pricingPlans }: PricingTabProps) {
  ...
  const handleDeletePricingPlan = async (planId: string) => {
    ...
    toast.promise(deletePricingPlanAction(planId), {
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-[10px] sm:text-[11px] text-zinc-400 font-mono mt-0.5">
            Manage pricing options shown on the landing page
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {pricingPlans.map((plan) => (
          <div
            key={plan.id}
            className={`bg-white border rounded-2xl p-5 dark:bg-zinc-950 shadow-sm flex flex-col justify-between ${plan.isPopular ? "border-purple-500" : "border-zinc-200 dark:border-zinc-800"}`}
          >
            <div>
              <div className="flex items-start justify-between gap-2 mb-2">
                <h3 className="text-base font-bold text-zinc-900 dark:text-zinc-50">{plan.name}</h3>
                {plan.isPopular && (
                  <span className="px-2 py-0.5 rounded bg-purple-100 dark:bg-purple-950 text-purple-700 dark:text-purple-300 font-mono text-[9px] uppercase tracking-wider font-extrabold">
                    Popular
                  </span>
                )}
              </div>
