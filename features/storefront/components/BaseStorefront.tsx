...
export default function BaseStorefront({
  org,
  products,
  className = "min-h-screen bg-zinc-50 text-zinc-900 dark:bg-zinc-950 dark:text-zinc-50 font-sans pb-24",
  style,
  renderHero,
  renderProfile,
  preProductsSlot,
  ...
  return (
    <div className={className} style={style}>
      ...
      {/* Extra slots like categories */}
      {preProductsSlot}

      {/* 3. Products Grid */}
      {products.length > 0 && (
        <section className={productsSectionClassName}>
          {!renderHero && !renderProfile && (
            <h2 className="text-lg font-bold tracking-tight mb-6">
              Products & Services
              <span className="ml-2 text-xs font-mono text-zinc-400">({products.length})</span>
            </h2>
          )}
                  ...
                  <Link href={`/products/${product.id}`} className="flex-1 flex flex-col group">
                    ...
                    <div className="p-4 flex-1 flex flex-col justify-between">
                      <div>
                        ...
                        <h3 className="text-sm font-bold text-zinc-900 dark:text-zinc-50 leading-snug line-clamp-1 mt-1 group-hover:text-purple-600 dark:group-hover:text-purple-400 transition-colors">
                          {product.name}
                        </h3>
