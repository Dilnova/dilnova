...
export default function ManageProductsClient({
  ...
  const handleUpdateStock = async (id: string, itemName: string) => {
    ...
    toast.promise(
      quickUpdateProductStockAction({ productId: id, newQuantity: qty })
        .then((result) => {
    ...
    <div className="space-y-4 sm:space-y-5">
      ...
      <div className="grid grid-cols-3 gap-2 sm:gap-3">
        <div className="bg-white border border-zinc-200 dark:bg-zinc-950 dark:border-zinc-800 rounded-xl p-3 sm:p-4 text-center">
          <p className="text-lg sm:text-2xl font-extrabold text-zinc-900 dark:text-zinc-50 font-mono">
            {products.length}
          </p>
        ...
        <div className="bg-white border border-zinc-200 dark:bg-zinc-950 dark:border-zinc-800 rounded-xl p-3 sm:p-4 text-center">
          <p className="text-lg sm:text-2xl font-extrabold text-indigo-600 dark:text-indigo-400 font-mono">
            {productCount}
          </p>
          <p className="text-[10px] sm:text-xs text-zinc-500 dark:text-zinc-400 font-medium mt-0.5">
            Products
        ...
        <div className="bg-white border border-zinc-200 dark:bg-zinc-950 dark:border-zinc-800 rounded-xl p-3 sm:p-4 text-center">
          <p className="text-lg sm:text-2xl font-extrabold text-emerald-600 dark:text-emerald-400 font-mono">
            {serviceCount}
          </p>
          <p className="text-[10px] sm:text-xs text-zinc-500 dark:text-zinc-400 font-medium mt-0.5">
            Services
      ...
      {(() => {
        ...
        const barColor = isAtLimit ? "bg-rose-500" : isNearLimit ? "bg-amber-400" : "bg-indigo-500";
