...
const stockAvailabilityDefinitionSchema = z.object({
  id: z
    ...
    .min(1)
    .max(50)
    .regex(/^[a-z0-9_]+$/, "Status ID must be lowercase letters, numbers, and underscores."),
...
export const updateStockAvailabilityCatalogSchema = z.object({
  options: z
    .array(stockAvailabilityDefinitionSchema)
    .min(1, "At least one stock availability option is required."),
});
...
export const updateImsLicenseSchema = z.object({
  ...
  imsMaxListingCount: z
    .number()
    .int("Must be a whole number.")
    .min(1, "Minimum is 1.")
    .max(100_000, "Maximum is 100,000.")
    .optional(),
});
