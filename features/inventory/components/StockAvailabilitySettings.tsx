  ...
  const handleSave = () => {
    startTransition(async () => {
      try {
        ...
        toast.error(
          err instanceof Error ? err.message : "Failed to save stock availability catalog.",
        );
      }
    });
  };
    ...
    <div className="bg-white border border-zinc-200 rounded-xl sm:rounded-2xl p-4 sm:p-5 dark:bg-zinc-950 dark:border-zinc-800 shadow-sm space-y-4">
      <div>
        ...
        <p className="text-[10px] text-zinc-400 mt-1 leading-relaxed">
          Define stock status labels vendors can assign to products — e.g. In Stock, Out of Stock,
          Pre-Order.
        </p>
      </div>
        ...
        {catalog.map((option) => (
          ...
          >
            ...
            <div className="flex items-center gap-3 shrink-0">
              ...
              <ToggleButton
                enabled={option.platformEnabled}
                onToggle={() => togglePlatformEnabled(option.id)}
              />
            </div>
          </div>
        ))}
