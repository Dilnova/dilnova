...
export async function uploadAndSubmitPaymentSlipAction(formData: FormData) {
  return runWithCorrelationId(async () => {
    ...
    const parsed = uploadPaymentSlipFormSchema.safeParse({ orderId });
    if (!parsed.success) {
      return {
        success: false as const,
        error: parsed.error.issues[0]?.message || 'Invalid order ID.',
    ...
    const contentType =
      resolvePaymentSlipExtension(file.type) ??
      resolvePaymentSlipExtensionFromFilename(file.name);
    if (!contentType) {
      return {
        success: false as const,
        error: 'Please upload an image file (JPG, PNG, WebP, or GIF).',
    ...
    try {
      ...
      const storagePath = await uploadPaymentSlipToStorage({
        orderId: order.id,
        bytes: buffer,
        contentType,
      });
      ...
      logger.error('Failed to upload payment slip via server action', { orderId: order.id, error });
      return {
        success: false as const,
        error: error instanceof Error ? error.message : 'Failed to save payment slip.',
      };
    }
  });
}
