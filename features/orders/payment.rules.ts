...
export function canUploadPaymentSlip(order: { paymentMethod: string; status: string }): boolean {
  return isBankTransferPayment(order.paymentMethod) && order.status === "pending_payment";
}
...
export function canFulfillCodOrder(order: { paymentMethod: string; status: string }): boolean {
