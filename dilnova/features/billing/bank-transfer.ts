...
export function parseBankTransferDetailsFromMetadata(
  ...
  const accountName = typeof metadata.bankAccountName === 'string' ? metadata.bankAccountName.trim() : '';
  const accountNumber =
    typeof metadata.bankAccountNumber === 'string' ? metadata.bankAccountNumber.trim() : '';
  const branchCode = typeof metadata.bankBranchCode === 'string' ? metadata.bankBranchCode.trim() : '';
  const instructions =
