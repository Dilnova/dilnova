...
describe("orderPayment helpers", () => {
  it("allows slip upload only while payment is pending", () => {
    ...
    expect(
      canUploadPaymentSlip({ paymentMethod: "cash_on_delivery", status: "pending_payment" }),
    ).toBe(false);
  });
  ...
  it("allows COD fulfillment from pending payment", () => {
    expect(
      canFulfillCodOrder({ paymentMethod: "cash_on_delivery", status: "pending_payment" }),
    ).toBe(true);
    expect(canFulfillCodOrder({ paymentMethod: "bank_transfer", status: "pending_payment" })).toBe(
      false,
    );
  });
});
