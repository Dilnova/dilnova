...
describe("validateServerEnv", () => {
  ...
  it("validates DATABASE_POOL_SIZE when present", async () => {
    ...
    // Valid cases
    expect(
      productionServerEnvSchema.safeParse({ ...baseEnv, DATABASE_POOL_SIZE: "10" }).success,
    ).toBe(true);
    expect(
      productionServerEnvSchema.safeParse({ ...baseEnv, DATABASE_POOL_SIZE: undefined }).success,
    ).toBe(true);
    ...
    const parseInvalid = (val: string) => {
      ...
      if (!res.success) {
        expect(res.error.flatten().fieldErrors.DATABASE_POOL_SIZE).toEqual([
  ...
  it("validates SENTRY_TRACES_SAMPLE_RATE when present", async () => {
    ...
    // Valid cases
    expect(
      productionServerEnvSchema.safeParse({ ...baseEnv, SENTRY_TRACES_SAMPLE_RATE: "0.1" }).success,
    ).toBe(true);
    expect(
      productionServerEnvSchema.safeParse({ ...baseEnv, SENTRY_TRACES_SAMPLE_RATE: "0" }).success,
    ).toBe(true);
    expect(
      productionServerEnvSchema.safeParse({ ...baseEnv, SENTRY_TRACES_SAMPLE_RATE: "1" }).success,
    ).toBe(true);
    expect(
      productionServerEnvSchema.safeParse({ ...baseEnv, SENTRY_TRACES_SAMPLE_RATE: undefined })
        .success,
    ).toBe(true);

    // Invalid cases
    const parseInvalid = (val: string) => {
      const res = productionServerEnvSchema.safeParse({
        ...baseEnv,
        SENTRY_TRACES_SAMPLE_RATE: val,
      });
      expect(res.success).toBe(false);
      if (!res.success) {
        expect(res.error.flatten().fieldErrors.SENTRY_TRACES_SAMPLE_RATE).toEqual([
          "SENTRY_TRACES_SAMPLE_RATE must be a float between 0 and 1",
        ]);
      }
    };
