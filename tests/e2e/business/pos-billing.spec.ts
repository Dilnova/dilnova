...
test.describe('POS Billing Flow', () => {
  test('vendor can load register and ring up item', async ({ page }) => {
    ...
    const productCard = page.locator('button').filter({ hasText: /Rs|LKR|\$/i }).first();
    ...
    const receiptModal = page.getByRole('dialog').or(page.locator('.modal')).or(page.getByText(/Receipt/i));
